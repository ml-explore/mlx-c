...
inline mlx_fast_metal_kernel_private_& mlx_fast_metal_kernel_get_(mlx_fast_metal_kernel d) {
...
extern "C" int mlx_fast_metal_kernel_add_output_arg(mlx_fast_metal_kernel cls, const int* shape, size_t size, mlx_dtype dtype) {
  try {
    mlx_fast_metal_kernel_get_(cls).output_shapes.push_back(std::vector<int>(shape, shape+size));
    mlx_fast_metal_kernel_get_(cls).output_dtypes.push_back(mlx_dtype_to_cpp(dtype));
...
extern "C" int mlx_fast_metal_kernel_set_thread_group(mlx_fast_metal_kernel cls, int thread1, int thread2, int thread3) {
  try {
    mlx_fast_metal_kernel_get_(cls).thread_group = std::make_tuple(thread1, thread2, thread3);
  } catch (std::exception& e) {
    mlx_error(e.what());
    return 1;
  }
...
extern "C" int mlx_fast_metal_kernel_add_template_arg_dtype(mlx_fast_metal_kernel cls, const char* name, mlx_dtype dtype) {
  try {
    mlx_fast_metal_kernel_get_(cls).template_args.push_back(std::make_pair(std::string(name), mlx_dtype_to_cpp(dtype)));
  } catch (std::exception& e) {
    mlx_error(e.what());
    return 1;
  }
...
extern "C" int mlx_fast_metal_kernel_add_template_arg_int(mlx_fast_metal_kernel cls, const char *name, int value) {
  try {
    mlx_fast_metal_kernel_get_(cls).template_args.push_back(std::make_pair(std::string(name), value));
  } catch (std::exception& e) {
    mlx_error(e.what());
    return 1;
  }
...
extern "C" int mlx_fast_metal_kernel_add_input_name(mlx_fast_metal_kernel cls, const char* name) {
  try {
...
extern "C" int mlx_fast_metal_kernel_set_input_names(mlx_fast_metal_kernel cls, int num, ...) {
  try {
    ...
    for(int i = 0; i < num; i++) {
      mlx_fast_metal_kernel_get_(cls).input_names.push_back(va_arg(input_names, const char*));
    }
    va_end(input_names);
  } catch (std::exception& e) {
    mlx_error(e.what());
...
extern "C" int mlx_fast_metal_kernel_add_output_name(mlx_fast_metal_kernel cls, const char* name) {
  try {
...
extern "C" int mlx_fast_metal_kernel_set_output_names(mlx_fast_metal_kernel cls, int num, ...) {
  try {
    ...
    for(int i = 0; i < num; i++) {
      mlx_fast_metal_kernel_get_(cls).output_names.push_back(va_arg(output_names, const char*));
    }
    va_end(output_names);
  } catch (std::exception& e) {
    mlx_error(e.what());
...
extern "C" int mlx_fast_metal_kernel_apply(mlx_vector_array* outputs, mlx_fast_metal_kernel cls, const mlx_vector_array inputs, const mlx_stream stream) {
  try {
    if(!mlx_fast_metal_kernel_get_(cls).ctx) {
      mlx_fast_metal_kernel_get_(cls).ctx = mlx::core::fast::metal_kernel(mlx_fast_metal_kernel_get_(cls).name, mlx_fast_metal_kernel_get_(cls).input_names, mlx_fast_metal_kernel_get_(cls).output_names, mlx_fast_metal_kernel_get_(cls).source, mlx_fast_metal_kernel_get_(cls).header, mlx_fast_metal_kernel_get_(cls).contiguous_rows, mlx_fast_metal_kernel_get_(cls).atomic_outputs);
    }
    mlx_vector_array_set_(*outputs, mlx_fast_metal_kernel_get_(cls).ctx(mlx_vector_array_get_(inputs), mlx_fast_metal_kernel_get_(cls).output_shapes, mlx_fast_metal_kernel_get_(cls).output_dtypes, mlx_fast_metal_kernel_get_(cls).grid, mlx_fast_metal_kernel_get_(cls).thread_group, mlx_fast_metal_kernel_get_(cls).template_args, mlx_fast_metal_kernel_get_(cls).init_value, mlx_fast_metal_kernel_get_(cls).verbose, mlx_stream_get_(stream)));
  } catch (std::exception& e) {
    mlx_error(e.what());
    return 1;
  }
...
int mlx_fast_metal_kernel_add_input_name(mlx_fast_metal_kernel cls, const char* name);
int mlx_fast_metal_kernel_set_input_names(mlx_fast_metal_kernel cls, int num, ...);
int mlx_fast_metal_kernel_add_output_name(mlx_fast_metal_kernel cls, const char* name);
...
int mlx_fast_metal_kernel_apply(mlx_vector_array* outputs, mlx_fast_metal_kernel cls, const mlx_vector_array inputs, const mlx_stream stream);
        """
        )
