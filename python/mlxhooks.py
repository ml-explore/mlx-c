def mlx_load_reader(f, implementation):
    if implementation:
        print(
            """extern "C" int
mlx_load_file(mlx_array* res, FILE* in_stream, const mlx_stream s) {
   ...
   return 0;
    ...
    return True  # do not skip


def mlx_save_writer(f, implementation):
    if implementation:
        print(
            """extern "C" int
mlx_save_file(FILE* out_stream, const mlx_array a) {
   try {
     mlx::core::save(std::make_shared<CFILEWriter>(out_stream), mlx_array_get_(a));
   } catch (std::exception& e) {
     mlx_error(e.what());
     return 1;
   }
   return 0;
        ...
        print("""int mlx_save_file(FILE* out_stream, const mlx_array a);""")
    return True  # do not skip


def mlx_load_safetensors_reader(f, implementation):
    if implementation:
        print(
...
def mlx_save_safetensors_writer(f, implementation):
    if implementation:
        print(
  ...
  try {
    mlx::core::save_safetensors(
        std::make_shared<CFILEWriter>(out_stream),
